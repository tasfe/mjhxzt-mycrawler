...
public class TaobaoLucene {
	...
	private static Document getDoc(TSeed seed) {
		...
		FieldType LongIsType = new FieldType();
		LongIsType.setStored(true);
		LongIsType.setIndexed(true);
		LongIsType.setNumericType(NumericType.LONG);
		...
		Field fSeed = new Field("seed", seed.getSeed(), StrIsType);
		doc.add(fSeed);
		Field key = new Field("key", seed.getKey(), StrIsType);
		doc.add(key);
		Field lastVisitTime = new Field("lastVisitTime", seed.getLastVisitTime().getTime()+"", LongIsType);
	...
	public List<TLink> get(String key) throws Exception {
		...
		if (CollectionUtils.isEmpty(result)) {
			String seedUrl = "http://s.taobao.com/search?q="
					+ key
					+ "&suggest=0_1&wq=aodi&suggest_query=aodi&source=suggest&initiative_id=tbindexz_20131028&spm=1.6659421.754896237.1&sourceId=tb.index&search_type=item&ssid=s5-e&commend=all";
			
			List<Object> crawlersLocalData = new CoreController().crawling(seedUrl, RelativeWordsCrawler.class);

			result = new ArrayList<TLink>();
			if (CollectionUtils.isNotEmpty(crawlersLocalData)) {
				result = (List<TLink>) crawlersLocalData.get(0);
				if (CollectionUtils.isNotEmpty(result)) {
					seed.setTLinkses(result);
					seed.setLastVisitTime(new Date());
					seed.setSeed(seedUrl);
					save(seed);
				}
